...
class binaryEncoder(_encoder):
    ...
    def binary_values(self, binary_values):
        if binary_values:
            ...
            if not isinstance(binary_values, (list, tuple)) and len(binary_values) != 2:
                raise ValueError("The 'binary_values' arguement has to be a list of 2 integers (e.g. [0,1], [-1,1]) etc)")
            ...
            print('Use default binary_values. Map label values to 0 (negative class) and 1 (positive class).')
    ...
    def fit(self,
        ...
        if not self.pos_label:
            ...
            if len(classes) > 2:
                warnings.warn(f"Warning! More than 2 unique values found. No pos_label is specified. Use '{classes[-1]}' as the positive class.")
...
class multiLabelEncoder(_encoder):
    ...
    def __init__(self):
        ...
        self._unrecognized_value_mapping = None # map unseen labels to the specified values
    ...
    def transform(self, y: List[List[Union[str, int]]]) -> List[int]:
        ...
        try:
            result = [sorted(set(self._encoder_dict.get(v, self.classes_.index(self._unrecognized_value_mapping)) for v in l)) for l in y]
        except ValueError as e:
            raise ValueError('Unrecognized label! Please specify "unrecognized_value_mapping" argument to map unseen label to a specified value.')
    ...
    def inverse_transform(self, y: List[int]) -> List[str]:
        ...
        result = [[self._decoder_dict.get(v, self._unrecognized_value_mapping) for v in l] for l in y]
