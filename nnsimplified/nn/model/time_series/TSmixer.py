...
class _BN2d(torch.nn.BatchNorm2d):
    ...
    def __init__(
        ...
        super().__init__(
            num_features=1,  # for this implementaion we don't use
            eps=eps,
            momentum=momentum,
            affine=affine,
...
class _TimeMixing(torch.nn.Module):
    ...
    def __init__(
        ...
        batchnorm2d_setting: Optional[dict] = {
            "eps": 1e-05,
            "momentum": 0.1,
            "affine": True,
            "track_running_stats": True,
        ...
        self._init_args = {
            k: v for k, v in locals().items() if k in _get_call_args(self.__class__)
        }
        ...
        self.linear = _initialize_linear_layer(
            in_features=in_time_steps,
        ...
        self.activation = (
            _construct_activation(activation_setting) if activation_setting else None
        )
...
class _FeatureMixing(torch.nn.Module):
    ...
    def __init__(
        ...
        if self.out_features == self.in_features:
            ...
            self.residual_conn = _initialize_linear_layer(
        ...
        # construct and register linear layers
        self.linear1 = _initialize_linear_layer(
            in_features=self.in_features,
            out_features=self.hidden_layer_size,
            weight_init=self.weight_init,
        )
        self.linear2 = _initialize_linear_layer(
...
class _FeatMixConditional(torch.nn.Module):
    ...
    def __init__(
        ...
        self.fmix_dynamic = _FeatureMixing(
        ...
        self.fmix_static = _FeatureMixing(
            in_features=static_features,
            hidden_layer_size=hidden_layer_size,
            out_features=in_features,  # force to have the same shape as the time series input
            activation_setting=activation_setting,
            batchnorm2d_setting=batchnorm2d_setting,
...
class _Mixer(nnModule):
    ...
    def __init__(
        ...
        self.time_mixer = _TimeMixing(
            in_time_steps=in_time_steps,
            activation_setting=activation_setting,
            batchnorm2d_setting=batchnorm2d_setting,
            bn_before_or_after=bn_before_or_after,
            weight_init=weight_init,
        ...
        self.feature_mixer = _FeatureMixing(
...
class _MixerConditional(nnModule):
    ...
    def __init__(
        ...
        self.feature_mixer = _FeatMixConditional(
            in_features=in_features,
    ...
    def input_shape(self):
        ...
        return torch.Size([-1, self.in_time_steps, self.in_features]), torch.Size(
            [-1, self.in_time_steps, self.static_features]
        )
    ...
    def forward(self, x):
        ...
        X = self.time_mixer(X)
        X = torch.transpose(X, dim0=-1, dim1=-2)
        y = self.feature_mixer(
            (X, static)
        )  # conditional feature mixing requires both time series features and static features.
...
class TSmixer(nnModule):
    ...
    def __init__(
        ...
        if self.in_features == self.out_features:
            ...
            self.feature_projection_layer = _initialize_linear_layer(
                in_features=in_features,
                out_features=out_features,
                weight_init=self.weight_init,
            )
        ...
        self.temporal_projection_layer = _initialize_linear_layer(
            in_features=in_time_steps,
    ...
    def in_time_steps(self, in_time_steps: int):
        ...
        if not isinstance(in_time_steps, int):
            raise ValueError(
                f"<in_time_steps> has to be an integer >= 1. Got {in_time_steps} instead."
            )
    ...
    def out_time_steps(self, out_time_steps: int):
        ...
        if not isinstance(out_time_steps, int):
            raise ValueError(
                f"<out_time_steps> has to be an integer >= 1. Got {out_time_steps} instead."
            )
    ...
    def in_features(self, in_features: int):
        ...
        if not isinstance(in_features, int):
            raise ValueError(
                f"<in_features> has to be an integer >= 1. Got {in_features} instead."
            )
        elif in_features < 1:
            raise ValueError(
                f"<in_features> has to be an integer >= 1. Got {in_features} instead."
    ...
    def out_features(self, out_features: int):
        ...
        if not isinstance(out_features, int):
            raise ValueError(
                f"<out_features> has to be an integer >= 1. Got {out_features} instead."
            )
        elif out_features < 1:
            raise ValueError(
                f"<out_features> has to be an integer >= 1. Got {out_features} instead."
    ...
    def fmixer_hidden_size(self, fmixer_hidden_size: int):
        ...
        if not isinstance(fmixer_hidden_size, int):
            raise ValueError(
                f"<fmixer_hidden_size> has to be an integer >= 1. Got {fmixer_hidden_size} instead."
            )
    ...
    def time_mix_only(self, time_mix_only: bool):
        ...
        if not isinstance(time_mix_only, bool):
            raise ValueError(
                f"<time_mix_only> has to be either True or False. Got {time_mix_only} instead."
            )
...
class TSmixer_ext(TSmixer):
    ...
    def __init__(
        ...
        if self.other_dynamic_features > 0:
            self.align_other_dynamic = _FeatureMixing(
                in_features=other_dynamic_features,
        ...
        self.align_static = _FeatureMixing(
            in_features=static_features,
    ...
    def other_dynamic_features(self, other_dynamic_features: int):
        ...
        if not isinstance(other_dynamic_features, int):
            raise ValueError(
                f"<static_features> has to be an integer >= 1. Got {other_dynamic_features} instead."
            )
    ...
    def static_features(self, static_features: int):
        ...
        if not isinstance(static_features, int):
            raise ValueError(
                f"<static_features> has to be an integer >= 1. Got {static_features} instead."
            )
        elif static_features < 1:
            raise ValueError(
                f"<static_features> has to be an integer >= 1. Got {static_features} instead."
    ...
    def forward(self, x: torch.Tensor):
        ...
        y = self.temporal_projection_layer(
            y
        )  # project to the desired output time steps
        y = torch.transpose(y, dim0=-1, dim1=-2)
    ...
    def input_shape(self):
        ...
        if self.other_dynamic_features > 0:
            shape.append(
                torch.Size([-1, self.in_time_steps, self.other_dynamic_features])
            )
