...
class mlpBase(nnModule):
    ...
    def __init__(
        ...
        self.add_module(
            name="linear_layers",
            module=self.construct_linear_layers(mlp_layers=mlp_layers),
        )
        ...
        if batchnorm_setting:
            ...
            self.add_module(
                ...
                module=torch.nn.ModuleList(
                    [
                        torch.nn.BatchNorm1d(num_features=i, **batchnorm_setting)
    ...
    def dropout_rate(self, dropout_rate):
        ...
        if (dropout_rate >= 1) or (dropout_rate < 0):
            ...
            dropouts = [
                torch.nn.Dropout(p=dropout_rate) for i in self._mlp_layers[1:-1]
            ]
    ...
    def construct_linear_layers(self, mlp_layers) -> torch.nn.ModuleList:
        ...
        linear_layers = torch.nn.ModuleList(
            [
                torch.nn.Linear(
                    in_features=mlp_layers[i], out_features=mlp_layers[i + 1], bias=True
                )
        ...
        if nn_structure:
            ...
            if isinstance(nn_structure[0], torch.Size):
                ...
                self.add_module(
                    name="input_layer",
                    module=nnParallel(
                        module_list=nn_structure[0], output_combine_method="concat"
                    ),
                )
            else:
    ...
    def nn_structure(self, nn_structure):
        ...
        if self._nn_structure:
            ...
            if isinstance(input_layer, int):
                ...
                raise ValueError(
                    "Unknown structure found in the first layer input; must be an int or a list of int, a list of nn modules or a dict of nn modules with names."
                )
