import torch
from ..helper import _weight_init
...
def _construct_activation(activation_setting: str | dict[str, dict]):
    ...
    available = [
        ...
        if callable(v) and v.__module__ == "torch.nn.modules.activation"
    ]
    ...
    if isinstance(activation_setting, str):
        ...
        if not isinstance(given_config, dict):
            raise ValueError(
                "Activation function arguments must be specified in python dict format!"
            )
    else:
        raise ValueError(
            '<activation_setting> argument accepts only string (name of init method) or dict ({"name of activation": dictionary of activation args})'
        )

    if name not in available:
        raise ValueError(
            f"{name} is not a valid activation function. Available options: {available}"
        )
