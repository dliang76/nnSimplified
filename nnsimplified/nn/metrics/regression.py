...
supported_classification_metrics = torchmetrics.regression.__dict__['__all__']

def get_metrics(metrics_name: str, **kwargs) -> torchmetrics:
...
def metrics_collection(*args: str, **kwargs: dict) -> torchmetrics.MetricCollection:
    ...
    for name in args:
        metrics = get_metrics(metrics_name = name)
        metrics_list.append(metrics)

    for name, settings in kwargs.items():
        metrics = get_metrics(metrics_name = name, **settings)
        metrics_list.append(metrics)
