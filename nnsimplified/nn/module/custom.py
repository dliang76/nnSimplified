...
class Passthrough(baseModule):
    ...
    def shape(self, shape: torch.Size | list[torch.Size]):
        ...
        if isinstance(shape, torch.Size):
            self._shape = (
                torch.Size([-1]) + shape[1:]
            )  # replace the first dim with -1 to indicate arbitrary batch size

        elif isinstance(shape, (list, tuple)):
            for idx, i in enumerate(shape):
                if not isinstance(i, torch.Size):
                    raise ValueError(
                        "<shape> has to be either a list of torch.Size or torch.Size"
                    )
        ...
        if isinstance(self.shape, torch.Size):
            ...
            if isinstance(dtype, torch.dtype):
                ...
                if len(dtype) == 1:
                    ...
                    raise ValueError(
                        f"The number of dtypes ({len(dtype)}) in <dtype> does not match the number of allowed input tensors ({len(self.shape)}) in <shape>"
                    )

                self._dtype = tuple(dtype)
            else:
                raise ValueError(
                    "<dtype> has to be either a list of torch.dtype or torch.dtype"
                )
    ...
    def __repr__(self):
        return (
            f"{self.__class__.__name__}(shape = {self._shape}, dtype = {self._dtype})"
        )
    ...
    def _input_check(self, x):
        ...
        if shape != self.shape:
            raise RuntimeError(
                "Incorrect shape for the Passthrough module! The input requires shape {self.shape} but got {shape} instead."
            )
...
class TensorSum(torch.nn.Module):
    ...
    def weights(self, weights: float):
        if not weights:
            ...
            if not isinstance(weights, (list, tuple)):
                raise ValueError(
                    "<weights> has to either be None or a list of numbers."
                )
    ...
    def forward(self, x):
        ...
        if self.weights is not None:
            return torch.sum(
                self.weights.view(len(x), *([1] * (x.dim() - 1))) * x, dim=0
            )
        else:
            return x.sum(dim=0)
...
class TensorMean(TensorSum):
    ...
    def forward(self, x):
        ...
        if self.weights is not None:
            return torch.sum(
                self._normalized_weights.view(len(x), *([1] * (x.dim() - 1))) * x, dim=0
            )
        else:
            return x.mean(dim=0)
...
class Split(torch.nn.Module):
    ...
    def __init__(self, split_size_or_sections: int | list[int], dim: int):
        super().__init__()
        ...
        if isinstance(split_size_or_sections, int):
            ...
            for i in split_size_or_sections:
                if not isinstance(i, int):
                    raise ValueError(
                        "<split_size_or_sections> has to either be an int or a list of integers."
                    )
    ...
    def forward(self, x):
        return torch.split(
            tensor=x, split_size_or_sections=self.split_size_or_sections, dim=self._dim
        )
        ...
        if rep_sizes:
            if isinstance(rep_sizes, (list, tuple)):
                ...
                if self._rep_sizes[0] < 1:
                    raise ValueError(
                        "<rep_sizes> provided must be a list of integers > 0!"
                    )
            else:
                raise ValueError(
                    "<rep_sizes> provided must be either a list or a tuple!"
                )
        else:
