...
def _retrieve_module_name(module):
    ...
    for i in reversed(inspect.stack()):
        names = [
            var_name
            for var_name, var_val in i.frame.f_locals.items()
            if var_val is module
        ]
        if len(names) > 0:
            return names[0]
...
def _rename_duplicates(lst: list[str]):
    ...
    for s in lst:
        ...
        if s not in postfix_dict:
            ...
            postfix_dict[s] = deque([""])  # postfix is an empty string
...
class nnParallel(nnModule):
    ...
    def __init__(
        self,
        module_list: list[torch.nn.Module | baseModule | int | torch.Size]
        | dict[str, torch.nn.Module | baseModule | int | torch.Size] = [],
    ...
    def register_modules(
        self,
        module_list: list[nnModule | baseModule | int]
        | dict[str, nnModule | baseModule | int],
        copy_module: bool,
    ):
        ...
        if module_list:
            ...
            if isinstance(module_list, dict):
                module_list = list(
                    module_list.items()
                )  # Some risk: rely on python dictionary ordering behavior (ordered by insertion)
                ...
                for module in module_list:
                    if isinstance(module, int):
                        ...
                        module_name = _retrieve_module_name(
                            module
                        )  # get variable name if the module is assigned to a variable
                        if (
                            module_name
                            == inspect.getfullargspec(_retrieve_module_name).args[0]
                        ):
                ...
                names = _rename_duplicates(
                    names
                )  # add numbered postfix to duplicated names

                module_list = list(zip(names, module_list))

            else:
                raise ValueError(
                    "The <module_list> argument has to be either a list of modules or dict of named modules"
                )
            ...
            for name, module in module_list:
                if isinstance(module, torch._dynamo.eval_frame.OptimizedModule):
                    raise ValueError(
                        "Modules included cannont be of the type OptimizedModule (compiled). Please compile the resulting model only."
                    )
                ...
                if isinstance(module, int):
                    ...
                    raise RuntimeError(
                        "Every element in <module_list> has to be one of following: a torch.nn.Module, an int or a torch.Size."
                    )
    ...
    def structure_input_shape(self) -> tuple[torch.Size]:
        ...
        if not hasattr(self, "_structure_input_shape"):
            ...
            module_input_shape = flatten_list(
                [
                    module.input_shape if isinstance(module, baseModule) else None
            ...
            if module_input_shape:
                if len(module_input_shape) == 1:
                    self._structure_input_shape = module_input_shape[
                        0
                    ]  # return torch.Size if the list only has 1 element
                else:
                    self._structure_input_shape = tuple(
                        module_input_shape
                    )  # return tuple of torch.Size if multiple
            else:
                self._structure_input_shape = (
                    None  # return None if all module input_shape are None
                )
        ...
        if self.use_common_input:
            if shape:
                if isinstance(shape, tuple):
                    shape = [
                        s for s in shape if s is not None
                    ]  # remove any None from the list

                    if shape:
                        shape = shape[
                            0
                        ]  # retrieve the first input shape as the common input shape
        ...
        if not hasattr(self, "_structure_input_dtype"):
            ...
            if module_input_dtype:
                if len(module_input_dtype) == 1:
                    self._structure_input_dtype = module_input_dtype[
                        0
                    ]  # return torch.dtype if the list only has 1 element
                else:
                    self._structure_input_dtype = tuple(
                        module_input_dtype
                    )  # return tuple of torch.dtyp if multiple
            else:
                self._structure_input_dtype = (
                    None  # return None if all module input_dtype are None
                )
        ...
        if self.use_common_input:
            if dtype:
                if isinstance(dtype, tuple):
                    dtype = [
                        s for s in dtype if s is not None
                    ]  # remove any None from the list

                    if dtype:
                        dtype = dtype[
                            0
                        ]  # retrieve the first input dtype as the common input dtype
    ...
    def use_common_input(self, use_common_input: bool):
        if use_common_input:
            ...
            if not isinstance(input_shapes, tuple):
                input_shapes = [input_shapes]

            if not isinstance(input_dtypes, tuple):
            ...
            if len(set(input_shapes)) > 1:
                raise RuntimeError(
                    "The inputs to the sub-modules are of different dimensions! Cannot use a common input."
                )

            if len(set(input_dtypes)) > 1:
                raise RuntimeError(
                    "The inputs to the sub-modules are of different data types! Cannot use a common input."
                )

            if contain_unknown_shape:
                warnings.warn(
                    "Unable to determine the required input data shape for some of the modules. Since we cannot check the integrity of the structure, please proceed with caution.",
                    RuntimeWarning,
                )
    ...
    def output_combine_method(self, output_combine_method):
        ...
        if output_combine_method:
            # check output shape
            output_shapes = flatten_list(
                [
                    m.output_shape if hasattr(m, "output_shape") else None
                    for name, m in self.module_list
                ]
            )
            ...
            if None in output_shapes:
                ...
                if output_combine_method == "sum":
                    ...
                    if len(set(output_shapes)) > 1:
                        ...
                        raise RuntimeError(
                            "Outputs of the modules are of different shapes! Cannot be summed."
                        )
                elif output_combine_method == "concat":
                    if len(set([d[0:-1] for d in output_shapes])) > 1:
                        # mismatch in -1 dimension; cannot be concated
                        raise RuntimeError(
                            "Outputs of the modules are of different length! Cannot be concated."
                        )
...
class nnModular(nnModule):
    ...
    def __init__(
        ...
        network_structure: list[
            list[baseModule | torch.nn.Module] | baseModule | torch.nn.Module
        ],
        copy_module: bool = False,
        **kwargs,
    ):
    ...
    def register_modules(
        ...
        | dict[str, list[baseModule | torch.nn.Module] | baseModule | torch.nn.Module],
        copy_module: bool,
    ):
        ...
        for lvl, structure in enumerate(network_structure):
            if isinstance(structure, int):
                ...
                if len(structure) > 1:
                    lvl_module = nnParallel(
                        structure
                    )  # form nnParallel if the level structure contains more than 1 module
                else:
                    ...
                    if isinstance(lvl_module, int):
                        ...
                        lvl_module = Passthrough(
                            shape=torch.Size([-1, lvl_module])
                        )  # set first dim to -1 to indicate arbitrary batch size
                    elif isinstance(lvl_module, torch.Size):
                        # module is Passthrough if a torch.Size is passed.
                        lvl_module = Passthrough(
                            shape=torch.Size([-1]) + lvl_module[1:]
                        )  # change first dim to -1 to indicate arbitrary batch size

            else:
                raise ValueError(
                    "Elements of 'network_structure' has to be either a list of modules) or dict of named modules"
                )
    ...
    def _check_structure_integrity(self):
        ...
        if x is None:
            ...
            warnings.warn(
                "Unable to determine structure integrity because required input data shape cannot be determined! Proceed with caution."
            )
        else:
            ...
            for lvl, lvl_module in enumerate(self._level_modules):
                training_mode = (
                    lvl_module.training
                )  # record current training mode for the module
                ...
                try:
                    ...
                    if lvl == 0:
                        raise RuntimeError(
                            f"Error in connecting to Level {lvl} module! " + str(e)
                        )
                    else:
                        raise RuntimeError(
                            f"Error in connecting to Level {lvl - 1} to Level {lvl} module! "
                            + str(e)
                        )
