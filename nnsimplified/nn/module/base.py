...
class nnModule(baseModule, ABC):
        ...
        if not hasattr(self, "_output_shape"):
            ...
            if dummy_output is None:
                ...
                self._output_shape = tuple(
                    torch.Size([-1]) + i.shape[1:] for i in dummy_output
                )
    ...
    def _create_dummy_input(self, batch_size=4):
        ...
        if input_shape is None:
            ...
            input_shape = [
                input_shape
            ]  # force list for so we can use the same code to treat both single or multiple inputs
            ...
            if None in input_shape:
                ...
                if input_dtype is None:
                    ...
                    input_dtype = [
                        torch.float32 if i is None else i for i in input_dtype
                    ]
        ...
        for idx, d in enumerate(input_shape):
            ...
            if dtype in self.torch_float_type:
                ...
                input = torch.rand(shape, dtype=dtype, device=self.device)

            elif dtype in self.torch_int_type:
                # generate random int tensor if dtype is of int type
                input = torch.randint(
                    low=0, high=1, size=shape, dtype=dtype, device=self.device
                )
    ...
    def _create_dummy_output(self, batch_size=4):
        ...
        if dummy_input is not None:
            training_mode = (
                self.training
            )  # record current training mode (train or eval) so we can restore it later
    ...
    def visualize(
        self, input_data=None, graph_dir="TB", roll=True, expand_nested=True, **kwargs
    ):
        if input_data is None:
            ...
            if input_data is None:
                raise RuntimeError(
                    "Unable to generate mock input data for visualizing the network structure. Please provide <input_data>."
                )
